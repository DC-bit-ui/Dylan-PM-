# Strategy & OKRs

**Last updated:** 2026-04-28 (initial population from Cowork handoff — OKRs not yet captured)

---

## Vision (3+ years)
_(to confirm with Dylan — not provided in handoff)_

## Strategy (this period)
_(to confirm — inferred from active epics:)_
- Land **first Schedule 2 model validation run** (HORIZON regulatory readiness — AP-2116)
- Deliver **Frontier Phase 2** + property management (AP-1963, AP-2009) — primary internal-tooling investment
- Operationalise **Stormboy** — process alignment between Growth, Field, Product
- Stand up **crediting workflow template** for T1 Offsets Report (AP-2187 — Discovery)
- Operation KCT phase 1 (AP-1964 — partner-driven)

## Current OKRs
**Status:** _Q OKRs are still being decided for the quarter (confirmed by Dylan, 2026-04-28). Until they land, `/okr-check` will return "OKRs pending — alignment lens unavailable; treat strategic priorities (above) as the proxy."_

When OKRs are finalised, populate this section and update `/okr-check` to reactivate the alignment lens.

### Objective 1: _(pending)_
- **KR1.1:** _(measurable)_ — current: _(value)_ — target: _(value)_
- **KR1.2:** …

---

## Where Dylan owns / contributes
- **Owns (PM):** Frontier (AP-1963, AP-2009), Stormboy alignment, T1 Offsets Report PRD-side
- **Contributes (PM support):** HORIZON validation framework (AP-2116), KCT phase 1 (AP-1964), LawrieCo referrer view (AP-1965)

## Bets we're NOT making (this period)
_(populate as Dylan articulates the say-no list)_

## Recent strategic pivots
_(append-only log; populate as changes happen)_
